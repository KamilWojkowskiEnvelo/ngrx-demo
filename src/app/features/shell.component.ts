...
@Component({
  ...
  template: `
    <nav
      class="flex gap-4 px-8 py-6 mb-6 bg-emerald-300 dark:bg-slate-700 dark:text-white">
      <a
        exact
        routerLink="."
    ...
    <main class="px-8">
      <div *ngIf="user$ | async as user">
        ...
        <p>
          <span class="font-semibold mr-2">Użyj ciemnego motywu: </span
          >{{ user.settings.darkMode }}
        </p>
        <p>
          <span class="font-semibold mr-2">Zapisany do newslettera: </span
...
export class ShellComponent {
  ...
  public count$ = this.store.select(state => state.counter.current);
